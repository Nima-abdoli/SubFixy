...
namespace SubFixer
{
    public class SubFixer
    {
        ...
        // Hold Path of File Here
        private string mPath { get; set; }

        private Byte[] mFileByte { get; set; }


        public SubFixer(Byte[] FileByte)
        {
            // Get The path from caller and hold it in mPath
            mFileByte = FileByte;
        }
        ...
        public void Start()
        {
            ...
            foreach (Byte item in mFileByte)
            {
                if (item > 127)
                {
                    Decoding(Hex.ToString("2x"))
                }
            }
